...
En Genesis Plaza el board `LIVE` viene con un cartel 3D que alterna entre
`live_events.glb` y `next_live_events.glb` según haya o no eventos en vivo.

**Está desactivado por defecto en este paquete** porque esos `.glb` tienen las
coordenadas de Genesis Plaza horneadas en la geometría: en otra escena aparecen
en un lugar arbitrario. Si los querés, activalos y posicionalos a mano:
...
Si no los usás, podés borrar `assets/models/out/models/` del paquete (–56 KB).
